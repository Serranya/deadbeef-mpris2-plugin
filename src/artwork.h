...
// returns filename of cached image, or NULL
	char* (*get_album_art)(const char *fname, const char *artist,
			const char *album, int size, artwork_callback callback,
			void *user_data);
...
// synchronously get filename
	char* (*get_album_art_sync)(const char *fname, const char *artist,
...
// NOTE: this function is deprecated, please use make_cache_path2
	void (*make_cache_path)(char *path, int size, const char *album,
			const char *artist, int img_size);

// creates full path string for cache storage
	int (*make_cache_path2)(char *path, int size, const char *fname,
			const char *album, const char *artist, int img_size);
} DB_artwork_plugin_t;
